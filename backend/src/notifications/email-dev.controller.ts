...
// (helmet's default CSP would otherwise strip them). Dev-only endpoint.
const PREVIEW_CSP = "default-src * data: blob: 'unsafe-inline'";

/**
 * DEV-ONLY email playground. Preview every template in the browser and send a
 * test to any address. Returns 404 in production. Mounted under /api/email/dev.
 */
...
export class EmailDevController {
  ...
  async send(
    ...
    this.assertDev();
    if (!to || !TYPES.includes(type)) return resultHtml({ ok: false, msg: 'Missing or invalid email/type.' });
    const enabled = this.config.get<string>('EMAIL_ENABLED') === 'true';
    await this.enqueue.execute({
      ...this.samplePayload(type, locale, to),
      // unique per click so dev test-sends are never deduped away
    ...
    return resultHtml({
      ...
      msg: enabled
        ...
        : `Enqueued "${type}" (${locale}) to ${to}, but EMAIL_ENABLED is false → logged, NOT sent. Set EMAIL_ENABLED=true (+ verified domain) to deliver.`,
    });
  }
}

// ── tiny HTML helpers (dev tool) ────────────────────────────────────────────
...
function indexHtml(enabled: boolean): string {
  const rows = TYPES.map((type) => {
    const previews = LOCALES.map(
      (l) =>
        `<a href="/api/email/dev/preview?type=${type}&locale=${l}" target="_blank" style="display:inline-block;margin-right:8px;padding:7px 12px;border:1px solid #d4cfc9;border-radius:8px;color:#f7f5f2;text-decoration:none;font-size:12px;">Preview ${l.toUpperCase()} ↗</a>`,
    ).join('');
  ...
  const banner = enabled
    ...
    : `<div style="background:rgba(201,58,57,.15);border:1px solid #C93A39;color:#f7b0af;padding:12px 14px;border-radius:10px;font-size:12px;margin-bottom:28px;">EMAIL_ENABLED=false — test sends are LOGGED, not delivered. Previews work either way.</div>`;
...
function resultHtml(r: { ok: boolean; msg: string }): string {
  return shell(`
    ...
    <a href="/api/email/dev" style="display:inline-block;margin-top:24px;color:#f7b0af;font-size:12px;">← Back</a>`);
}
