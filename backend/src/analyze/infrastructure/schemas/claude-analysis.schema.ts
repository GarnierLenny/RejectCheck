/**
 * JSON schema Claude must conform to via tool_use.
 *
 * buildAnalysisTool() (submit_analysis): the single-pass full analysis —
 * scores, per-source audits with inline fix blocks, seniority/tone with fixes,
 * bullet reviews, ATS keywords, highlight terms, and optionally
 * project_recommendation. This is the only analysis tool; the old hot+deep
 * two-pass tools were removed. Old DB rows that still hold the two-pass shape
 * are handled at read time by mergeHotAndDeep in the DTO layer.
 *
