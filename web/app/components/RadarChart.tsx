...
export function RadarChart({
  ...
  // ── Shared pieces (composed differently for fluid vs fixed layouts) ─────────
  const legendEl =
    hasExpected && legend ? (
  ...
  const evidenceHead = evidenceHeader ? (
    <div className="mb-4">
  ...
  const evidenceFoot = evidenceFooter ? <div className="mt-6">{evidenceFooter}</div> : null;

  // Fluid: radar gets its own full-width row (enlarged, centered); the per-axis
  // breakdown bars sit below it, in a two-column grid on desktop.
  if (fluid) {
    return (
      <div className="flex flex-col gap-8">
        <div className="mx-auto w-full max-w-[600px] p-2">
          {legendEl}
          {svg}
        </div>
        {showEvidence && (
          <div>
            {evidenceHead}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">{axisRows}</div>
            {evidenceFoot}
          </div>
        )}
      </div>
  ...
  // Fixed size: legacy side-by-side layout (radar left, breakdown right).
  return (
