...
export function TechnicalRadarChart({ data, isPremium = true }: Props) {
  const gapCount = data.skills.filter(s => s.expected > s.current).length;
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="w-[210px] shrink-0">
            {isPremium ? (
              <div className="bg-rc-surface border border-rc-border rounded p-5 flex flex-col gap-4">
                <div>
              ...
              <BlurredSection
                aggregateText={`${data.skills.length} skills ranked by job priority`}
                ctaText="See skill ranking →"
              >
                <div className="bg-rc-surface border border-rc-border rounded p-5 flex flex-col gap-4">
                  <div>
                    <h4 className="font-mono text-[12px] uppercase tracking-widest text-rc-red font-bold flex items-center gap-1.5 mb-0.5">
                  ...
                  <ol className="flex flex-col gap-1.5">
                    {data.skills.map((s, i) => {
                      const rankColors = ["text-rc-red", "text-rc-amber", "text-rc-muted", "text-rc-hint", "text-rc-hint"];
                      return (
                        <li key={i} className="flex items-center gap-3 p-2.5 rounded bg-rc-bg border border-rc-border/50">
                          <span className={`font-mono text-[13px] font-bold w-5 text-center shrink-0 ${rankColors[i]}`}>{i + 1}</span>
                          <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                            <span className="font-mono text-[11px] text-rc-text leading-tight">{s.name}</span>
                          </div>
                        </li>
                      );
                    })}
        ...
        <div className="space-y-8">
          ...
          <BlurredSection
            aggregateText={`${gapCount} critical gap${gapCount !== 1 ? "s" : ""} identified across ${data.skills.length} skills — see your full technical breakdown`}
            ctaText="Unlock full analysis →"
          >
            <div className="space-y-4">
              <div className="bg-rc-surface border border-rc-border rounded p-8">
                <h3 className="font-mono text-[12px] uppercase tracking-[0.2em] text-rc-red mb-4 font-bold">Strategic Recommendation</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {data.skills.map((s, idx) => (
                  <div key={idx} className="bg-rc-surface border border-rc-border rounded p-6 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[11px] uppercase tracking-wider font-semibold text-rc-text">{s.name}</span>
                      <span className="px-2.5 py-1 rounded text-[9px] font-bold bg-rc-amber/10 text-rc-amber border border-rc-amber/20">GAP</span>
                    </div>
                    <div className="h-3 bg-rc-border/40 rounded w-3/4" />
                    <div className="space-y-1">
                      {[100, 70].map((w, i) => (
                        <div key={i} className="h-2.5 bg-rc-border/30 rounded" style={{ width: `${w}%` }} />
                      ))}
                    </div>
                  </div>
                ))}
