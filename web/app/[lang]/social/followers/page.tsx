...
import { useMyFollowers } from "../../../../lib/queries";
import { useMarkFollowersSeen } from "../../../../lib/mutations";

function MyFollowersContent() {
  const router = useRouter();
  const { user, loading } = useAuth();
  const { localePath, t } = useLanguage();
  const markSeen = useMarkFollowersSeen();
      ...
      <div className="max-w-[700px] mx-auto px-5 md:px-[40px] py-12">
        <header className="mb-6 border-b border-rc-border pb-4">
          <Heading as="h1" size="lg">
            {t.publicProfilePage.lists.myFollowers}
          </Heading>
